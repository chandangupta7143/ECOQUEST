...
// Auto-create uploads directory (fixes ENOENT on fresh clones / local dev)
...
/**
 * Sanitize filename — strips Hindi/Unicode chars & spaces to prevent
 * Windows ENOENT errors (e.g. "अध्याय 1.docx" → "file.docx")
 */
function sanitizeFilename(prefix, originalName) {
  const ext  = path.extname(originalName) || '';
  const base = path.basename(originalName, ext)
    .replace(/[^\x00-\x7F]/g, '')   // remove non-ASCII (Hindi, emoji)
...
const upload = multer({
  storage,
  limits: { fileSize: 10 * 1024 * 1024 }, // 10 MB for notes
